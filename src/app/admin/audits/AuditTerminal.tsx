...
import { 
  ...
  AlertTriangle, Search, X, Database, 
  Lock, Eye, RefreshCw 
} from "lucide-react";
...
export default function AuditTerminal({ initialSecurityLogs, initialAuditLogs, clientIp }: any) {
  ...
  // Simulación de "Live Feed" y Carga del Sistema
  useEffect(() => {
    const interval = setInterval(() => {
      setSystemLoad(Math.floor(Math.random() * 30) + 10); // Random 10-40%
    }, 2000);
  ...
  const logs = activeTab === 'security' ? initialSecurityLogs : initialAuditLogs;
    ...
    <div className="relative h-full flex flex-col p-4 md:p-6 gap-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-primary/20 pb-4 bg-background/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <Button variant="ghost" size="icon" onClick={handleRefresh} className={cn("text-primary hover:bg-primary/10", isRefreshing && "animate-spin")}>
        ...
        <div className="grid grid-cols-1 gap-1">
          <AnimatePresence mode="popLayout">
            {logs.map((log: any, i: number) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <StatusBadge status={activeTab === 'security' ? (log.event_type === 'INTRUSION_ATTEMPT' ? 'ERROR' : 'OK') : 'INFO'} />
                  
                  <div className="flex flex-col">
                    <span className="text-sm font-bold text-foreground group-hover:text-primary transition-colors">
                      {activeTab === 'security' ? log.event_type : log.action}
                    </span>
            ...
            >
              ...
              <div className="flex items-center justify-between p-4 border-b border-primary/20 bg-primary/5">
                <div className="flex items-center gap-2 text-primary font-mono text-sm">
                  ...
                  MEMORY_DUMP_0x{selectedLog.id.substring(0, 8)}
                </div>
                <button onClick={() => setSelectedLog(null)} className="text-muted-foreground hover:text-destructive transition-colors">
                  <X className="h-5 w-5" />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto font-mono text-xs space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-foreground">{selectedLog.id}</div>
                  </div>
                </div>
                ...
                {activeTab === 'audit' && (
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-primary/10">
                    <div>
                      ...
                      <pre className="text-[10px] text-muted-foreground">{JSON.stringify(selectedLog.old_data, null, 2)}</pre>
                    </div>
                    <div>
                      <span className="text-emerald-400 uppercase text-[10px] mb-2 block">New State</span>
                      <pre className="text-[10px] text-muted-foreground">{JSON.stringify(selectedLog.new_data, null, 2)}</pre>
                    </div>
                  </div>
                )}
              </div>
...
function StatusBadge({ status }: { status: 'OK' | 'ERROR' | 'INFO' }) {
  ...
  return (
    <span className={cn("px-2 py-1 rounded text-[10px] font-bold border font-mono", colors[status])}>
      [{status}]
    </span>
  );
}