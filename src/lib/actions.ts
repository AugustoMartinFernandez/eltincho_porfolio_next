...
async function recordAuditLog(action: string, table: string, recordId: string, oldData: any = null, newData: any = null) {
  try {
...
export async function approveTestimonial(id: string) {
  ...
  // Audit Log
  recordAuditLog("APPROVE", "testimonials", id, { is_approved: false }, { is_approved: true });
...
export async function toggleFeaturedTestimonial(id: string, currentState: boolean) {
  ...
  // Audit Log
  recordAuditLog("TOGGLE_FEATURED", "testimonials", id, { featured: currentState }, { featured: !currentState });
...
// --- SECURITY (Admin Validation & Logging) ---

export async function validateAdminEmail(email: string): Promise<boolean> {
  const adminEmail = process.env.ADMIN_EMAIL;
  if (!adminEmail) {
    console.error("Security Error: ADMIN_EMAIL environment variable is not configured.");
    return false;
  }
...
// 2. Actualizar la contraseña (Una vez logueado vía link)
export async function updatePassword(password: string) {
  ...
  // Log de éxito (crítico)
  recordSecurityEvent("PASSWORD_UPDATE_SUCCESS", { timestamp: new Date().toISOString() });

  return { success: true };
}

// --- DANGER ZONE (Admin - Reset Metrics) ---
export async function resetUmamiMetrics() {
  const websiteId = process.env.NEXT_PUBLIC_UMAMI_WEBSITE_ID;
  const apiKey = process.env.UMAMI_API_KEY;

  if (!websiteId || !apiKey) return { success: false, error: "Configuración incompleta" };

  try {
    const res = await fetch(`https://api.umami.is/v1/websites/${websiteId}/reset`, {
      method: "POST",
      headers: {
...
function parseUserAgent(ua: string) {
  const browser = /chrome|crios|crmo/i.test(ua) ? "Chrome" : /firefox|fxios/i.test(ua) ? "Firefox" : /safari/i.test(ua) ? "Safari" : /msie|trident/i.test(ua) ? "IE" : /edge/i.test(ua) ? "Edge" : "Unknown";
  const os = /windows/i.test(ua) ? "Windows" : /macintosh|mac os x/i.test(ua) ? "macOS" : /linux/i.test(ua) ? "Linux" : /android/i.test(ua) ? "Android" : /ios|iphone|ipad|ipod/i.test(ua) ? "iOS" : "Unknown";
...
export async function updateTechnology(id: string, formData: FormData) {
  ...
  const { data: oldData } = await authSupabase.from("technologies").select("*").eq("id", id).single();
...
export async function deleteTechnology(id: string) {
  ...
  const { data: oldData } = await authSupabase.from("technologies").select("*").eq("id", id).single();
  const { error } = await authSupabase.from("technologies").delete().eq("id", id);
...
export async function createExperience(formData: FormData) {
  ...
  const payload = {
    ...
    end_date: current ? null : (formData.get("end_date") ? formData.get("end_date") as string : null),
...
export async function updateExperience(id: string, formData: FormData) {
  ...
  const { data: oldData } = await authSupabase.from("experience").select("*").eq("id", id).single();
...
export async function updateEducation(id: string, formData: FormData) {
  ...
  const { data: oldData } = await authSupabase.from("education").select("*").eq("id", id).single();
...
export async function bulkDeleteProjects(projects: { id: string; image_url?: string }[]) {
  const authSupabase = await createAuthClient();
  const ids = projects.map(p => p.id);

  // 1. Borrar imágenes en paralelo
  await Promise.all(
    projects.map(p => p.image_url ? deleteProjectImage(p.image_url) : Promise.resolve())
  );
  ...
  await recordAuditLog("BULK_DELETE", "projects", "multiple", { count: projects.length });
  revalidatePath("/admin/projects");
  revalidatePath("/projects");
  return { success: true };
}