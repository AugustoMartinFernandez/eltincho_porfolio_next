        ...
        <div className="mt-auto pt-4 border-t border-border flex items-center justify-between">
          ...
          <div className="flex -space-x-2">
            {project.tech_stack && project.tech_stack.length > 0 ? (
              <>
                {project.tech_stack.slice(0, 3).map((tech, i) => (
                  <div 
                    ...
                    title={tech.name}
                  >
                    {/* Primera letra de la tecnología */}
                    {tech.name[0]} 
                  </div>
                ))}
              ...
              <span className="text-xs text-muted-foreground italic">Sin stack</span>
            )}
          </div>
          ...
          <div className="flex gap-1">
            {project.repo_url && (
              <a href={project.repo_url} target="_blank" rel="noreferrer" className="p-2 text-muted-foreground hover:text-foreground transition-colors rounded-md hover:bg-accent" title="Ver Código">
                <Github className="h-4 w-4" />
              </a>
            )}
            {project.demo_url && (
              <a href={project.demo_url} target="_blank" rel="noreferrer" className="p-2 text-muted-foreground hover:text-primary transition-colors rounded-md hover:bg-primary/10" title="Ver Demo">
...
// Subcomponente simple para el Badge de estado
function StatusBadge({ status }: { status: Project['status'] }) {
  ...
  return (
    <span className={cn("shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium border uppercase tracking-wider", styles[status] || styles.finished)}>
