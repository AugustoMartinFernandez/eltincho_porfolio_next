    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2">
        {education.length > 0 ? education.map((edu) => (
          <div key={edu.id} className="p-5 rounded-2xl bg-card border border-border hover:border-primary/40 transition-colors group flex flex-col h-full">
            ...
            <div className="mt-auto pt-4 border-t border-border/50 text-xs font-mono text-muted-foreground flex items-center gap-2">
              ...
              {formatDate(edu.start_date)} — {edu.end_date ? formatDate(edu.end_date) : "Actualidad"}
            </div>
          </div>
        )) : (
          <p className="text-muted-foreground italic col-span-full">No hay educación registrada.</p>
        )}
      </div>
          ...
          <div className="relative w-full max-w-3xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between p-4 border-b border-border bg-muted/30">
              <div className="flex items-center gap-2">
                ...
                <div>
                    <h3 className="font-bold text-sm leading-none">{selectedCert.degree_title}</h3>
                    <p className="text-xs text-muted-foreground">{selectedCert.institution_name}</p>
                </div>
              </div>
